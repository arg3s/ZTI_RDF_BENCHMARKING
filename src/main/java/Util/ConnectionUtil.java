...
public class ConnectionUtil {
	...
	/**
	 * Retrieves the number of explicit statements in a repository. The default implementation delegates to
	 * connection.size().
	...
	public static long numberOfExplicitStatements(RepositoryConnection repositoryConnection) throws Exception {
		...
		/*
		 * // Another approach is to get an iterator to the explicit statements // (by setting the includeInferred
		 * parameter to false) and then count them. long explicitStatements = 0; RepositoryResult<Statement>
		 * statements = repositoryConnection.getStatements(null, null, null, false); explicitStatements = 0; while
		 * (statements.hasNext()) { statements.next(); explicitStatements++; } statements.close(); return
		 * explicitStatements;
		 */
	}
	 ...
	 * <p/>
	 * No method for this is available through the Sesame API, so GraphDB uses a special context that is
	 * interpreted as an instruction to retrieve only the implicit statements, i.e. not explicitly asserted in the
	 * repository.
	 * @param repositoryConnection a connection to a repository
