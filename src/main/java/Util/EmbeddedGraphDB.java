...
public class EmbeddedGraphDB implements Closeable {
	...
	/**
	 * A convenience method to create a temporary repository and open a connection to it. When the connection is
	 * closed all underlying objects (Util.EmbeddedGraphDB and LocalRepositoryManager) will be closed as well. The
	 * temporary repository is created in a unique temporary directory that will be deleted when the program
	 * terminates.
	 ...
	 */
	public static RepositoryConnection openConnectionToTemporaryRepository(String ruleset)
			throws IOException,
			RepositoryException,
			RDFParseException,
		...
		// Create an instance of Util.EmbeddedGraphDB and a single repository in it.
		...
		// Wrap the connection in order to close the instance of Util.EmbeddedGraphDB on connection close
	 ...
	 * @param overrides a map of repository creation parameters that override the defaults, or null if none should
	 * be overridden
	 * @throws RDFParseException
	 * @throws IOException
	 * @throws RDFHandlerException
	...
	public void createRepository(String repositoryId, String repositoryLabel, Map<String, String> overrides)
		...
		Resource repositoryNode =
				Models.subject(graph.filter(null, RDF.TYPE, RepositoryConfigSchema.REPOSITORY)).orElse(null);
		...
		if (overrides != null) {
			Resource configNode =
					(Resource) Models.object(graph.filter(null, SailRepositorySchema.SAILIMPL, null)).orElse(null);
